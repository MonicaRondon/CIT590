...
def all_locations():
    ...
    for row in range(0, 5):
        # run the following code once using the value of 0
        # then again using the value of 1
        # etc... 2,3,4
        # then we start a SECOND loop
        for col in range(0, 5): 
            # run once with the col value of 0
            # then with 1... 
            list.append((row, col))
        # this is where the COL loop ends and we go back
    # to the NEXT row... 
    return list    
...
def make_move(location, direction):
    ...
    # we must set the current location to BLANK
    #1 - monica - LOAD the value at the current location in a variable
