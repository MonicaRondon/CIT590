...
public class GameControl {
    ...
    /**
     * Prints a welcome message, then calls methods to perform each
    ...
    void createPlayers() {
        ...
        Human hmn = new Human(humansName);
        players[0] = hmn;
        players[1] = new Player("Manny");
        players[2] = new Player("Moe");
        players[3] = new Player("Jack");
    }
    ...
    void controlPlay() { 
    	...
    	for (int i = 0; i < players.length; i++ ){
        	if (!passed[i] && players[i].offerCard(players)){
    ...
    void printResults() { 
    	...
    	for (Player p : players){
    		System.out.println( p.name + " has " + p.getScore() + " points. ");
        }
    	int playerindex = findWinningPlayer();
    	if (playerindex < 0){
    		System.out.println("Nobdy wins.");
    	} else {
    		System.out.println(players[playerindex].name + " wins. ");
    	}
    }
    ...
    int findWinningPlayer() { 
    	...
    	if (currentHighscore == potentialTiescore){
    		return -1;
    	}
